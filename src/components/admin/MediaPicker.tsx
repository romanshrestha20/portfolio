  ...
  return (
    <div className="sm:col-span-2">
      ...
      <div className="flex items-center justify-between gap-4">
        <span className="admin-label">Project image</span>
      ...
      <div className="mt-3 grid gap-5 border border-white/10 p-4 sm:grid-cols-[220px_1fr] sm:p-5">
        ...
        <div className="flex min-w-0 flex-col justify-between gap-5">
          ...
          <button type="button" onClick={() => setOpen(true)} className="admin-secondary w-fit"><ImagePlus className="h-4 w-4" />{selectedUrl ? "Change image" : "Choose image"}</button>
        </div>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-3 backdrop-blur-sm sm:p-8" onMouseDown={() => setOpen(false)}>
          <section role="dialog" aria-modal="true" aria-labelledby="media-picker-title" className="flex max-h-[88vh] w-full max-w-5xl flex-col border border-white/10 bg-[#101213] shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <header className="flex items-start justify-between gap-5 border-b border-white/10 p-5 sm:p-6">
              ...
              <button type="button" onClick={() => setOpen(false)} className="admin-icon" aria-label="Close media picker"><X className="h-4 w-4" /></button>
            </header>

            <div className="border-b border-white/10 p-4 sm:p-5">
              <label className="relative block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-600" /><input ref={searchRef} className="admin-input pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search filename, alt text, or tag" aria-label="Search media library" /></label>
            </div>
              ...
              {filteredAssets.length > 0 ? (
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                  {filteredAssets.map((asset) => {
                    ...
                    return (
                      <button key={asset.id} type="button" onClick={() => choose(asset)} className={`group overflow-hidden border text-left transition ${selected ? "border-[#b8ff47] bg-[#b8ff47]/5" : "border-white/10 hover:border-white/30"}`}>
                        <div className="relative aspect-[16/11] overflow-hidden bg-black/40">
                          ...
                          {selected && <span className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-[#b8ff47] text-black"><Check className="h-4 w-4" /></span>}
                        </div>
                ...
                <div className="flex min-h-64 flex-col items-center justify-center text-center"><FileImage className="h-7 w-7 text-zinc-700" /><p className="mt-4 text-sm text-zinc-500">{assets.length ? "No images match that search." : "The media library is empty."}</p><Link href="/admin/media" className="admin-primary mt-5">Upload an image</Link></div>
              )}
            </div>
