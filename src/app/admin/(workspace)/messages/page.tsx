...
export default async function MessagesPage() {
  ...
  return <div className="mx-auto max-w-5xl"><p className="admin-kicker">03 / Incoming transmissions</p><h1 className="admin-title">Messages</h1><p className="admin-subtitle">Contact requests received through the public portfolio.</p><div className="mt-10 border-t border-white/10">{messages.map((message) => <article key={message.id} className={`border-b border-white/10 py-7 ${message.read ? "opacity-60" : ""}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold text-white">{message.name}</p><a href={`mailto:${message.email}`} className="text-sm text-signal">{message.email}</a></div><time className="text-xs text-zinc-500">{new Date(message.created_at).toLocaleString()}</time></div><p className="mt-5 max-w-3xl whitespace-pre-wrap text-sm leading-7 text-zinc-300">{message.message}</p>{!message.read && <form action={markMessageRead} className="mt-5"><input type="hidden" name="id" value={message.id} /><button className="signal-button"><MailCheck className="h-4 w-4" />Mark as read</button></form>}</article>)}</div>{messages.length === 0 && <p className="mt-10 text-sm text-zinc-500">No messages yet.</p>}</div>;
}
